...
edition = "2021"
---
//! build-naive — cronet library build orchestration (single-file cargo script).
//!
...
fn main() {
    ...
    if raw.is_empty() {
        die("usage: build-naive <build|package|download-toolchain|env|print-config> [--target LIST] [--libc glibc|musl] [--export]");
    }
    let cmd = raw[0].clone();
